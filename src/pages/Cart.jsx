        ...
        <div className='max-padd-container py-10'>
                ...
                <div key={i} className='rounded-lg bg-white p-2 mb-3'>
                  <div className='flex items-center gap-x-3'>
                    <div className='items-start flex gap-6'>
                      <img src={productData.image[0]} alt="product image" className='w-16 sm:w-18 rounded' height={60} width={60}/>
                    </div>
                    <div className='flex flex-col w-full'>
                      <div className='flexBetween'>
                        <h5 className='h5 !my-0 line-clamp-1'>{productData.name}</h5>
                        <FaRegWindowClose className='cursor-pointer text-secondary' onClick={() => updateQuantity(item._id, item.size, 0)}/>
                      </div>
                      <p className='bold-14 my-0.5'>{item.size}</p>
                      <div className='flexBetween'>
                        <div className='flex items-center ring-1 ring-slate-900/5 overflow-hidden rounded-full bg-primary'>
          ...
          <div className='flex my-20'>
            <div className='w-full sm:w-[450px]'>
              <CartTotal   />
              <button disabled={getCartAmount() === 0} onClick={() => navigate('/place-order')} className='btn-secondary mt-7'>Proceed to Checkout</button>
            </div>
          </div>
        </div>
      </div>
