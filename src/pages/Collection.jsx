...
const Collection = () => {
  ...
  const applyFilter = () => {
    ...
    if(search && showSearch){
      filtered = filtered.filter((prod) => prod.name.toLowerCase().includes(search.toLowerCase()))
    }
    ...
    if(subCategory.length){
      filtered = filtered.filter((prod) => subCategory.includes(prod.subCategory))
    }

    return filtered
    //let filtered = products.filter((product) => category.includes(product.category)).filter((product) => subCategory.includes(product.category))
  }
      ...
      <div className='flex flex-col sm:flex-row gap-8 mb-16'>
        ...
        <div className='min-w-72 bg-primary p-4 pt-8 pl-6 lg:pl-12 rounded-r-xl'>
          ...
          <div className='pl-5 py-3 mt-4 bg-white rounded-xl'>
            ...
            <div className='flex flex-col gap-2 text-sm font-light'>
              {["Men", "Women", "Kids"].map((cat) => (
                <label key={cat} htmlFor="" className='flex gap-2 medium-14 text-gray-30'>
          ...
          <div className='pl-5 py-3 mt-6 bg-white rounded-xl'>
            ...
            <div className='flex flex-col gap-2 text-sm font-light'>
              {["Topwear", "Bottomwear", "Winterwear"].map((subCat) => (
                <label key={subCat} htmlFor="" className='flex gap-2 medium-14 text-gray-30'>
          ...
          <div className='px-4 py-3 mt-6 bg-white rounded-xl'>
              ...
              <select onChange={(e) => setSortType(e.target.value) } name="" id="" className='border border-slate-900/5 outline-none text-gray-30 medium-14 h-8 w-full rounded px-2'>
        ...
        <div className='bg-primary p-4 rounded-l-xl'>
          ...
          {/* pagination buttons */}
          <div className='flexCenter flex-wrap mt-14 gap-4 mb-10'>
              ...
              {Array.from({length: totalPages}, (_, index) => (
                <button key={index + 1} onClick={() => setCurrentPage(index + 1)} className={`${currentPage === index + 1 ? '!bg-tertiary text-white' : ''} btn-light !py-1 !px-3`}>{index + 1}</button>
              ))} 
              <button disabled={currentPage === totalPages} onClick={() => setCurrentPage(prev => prev + 1)} className={`${currentPage === totalPages && "opacity-50 cursor-not-allowed"} btn-secondary !py-1 !px-3`}>Next</button>
          </div>
        </div>
